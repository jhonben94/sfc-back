...
import com.py.sfc.app.services.ConceptoPuntosService;

@RestController
@RequestMapping(value = "/concepto-puntos")
@CrossOrigin(origins = "*")
public class ConceptoPuntosController {

	@Autowired
	private ConceptoPuntosService service;

	@PostMapping(path = "/paginado")
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<?> listarPaginado(@RequestBody PaginadoParam<ConceptoPuntos> param) {
		try {
			PaginadoResult<ConceptoPuntos> lista = service.listarPaginado(param);
	...
	@PostMapping("/")
	public ResponseEntity<?> agregarRecurso(@RequestBody ConceptoPuntos param) {
		Respuesta<ConceptoPuntos> respuesta = new Respuesta<>();

		try {
			ConceptoPuntos dato = service.insertarSinClavePrimaria(param);
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> modificarRecurso(@PathVariable("id") Integer id, @RequestBody ConceptoPuntos param) {
		Respuesta<ConceptoPuntos> respuesta = new Respuesta<>();
		try {
			param.setConceptoPunto(id);
	...
	public ResponseEntity<?> obtenerRecurso(@PathVariable("id") Integer id) {
		...
		try {
			ConceptoPuntos dato = service.obtener(id);
	...
	public ResponseEntity<?> listar() {
		Respuesta<List<ConceptoPuntos>> respuesta = new Respuesta<>();
		List<ConceptoPuntos> dato = new ArrayList<ConceptoPuntos>();
		try {
			dato = service.listar();
			if (dato == null) {
				respuesta.setDato(dato);
